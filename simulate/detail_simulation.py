...
# Multiprocessing worker uses the forked copy of this global.
_CMD_TRACE = None
_QUERY_HNSW = None
_QUERY_ARRAY = None
_QUERY_K = None
...
def _simulate_latency_slice(start_end):
    """Compute latency sum for a [start, end) slice of cmd_trace."""
    start, end = start_end
    latency = 0
...
if __name__ == "__main__":
    ...
    idx_path=gen_idx_name(dataset_name, idx_type,graph_degree=args.graph_degree,ef_construction=args.ef_construction)
    ...
    cache_mapping = None
    if not baseline:
        mapping_cfg_file_name = f"{dataset_name}_gd{args.graph_degree}_ef{args.ef_construction}.npy"
    ...
    for it in range(1, 11):
        ...
        # Reset cache container between iterations.
        # Some mapping implementations use deque; others use list. We preserve container type if possible.
        ...
        if dataset_name != "GloVe":
            hnsw_idx.ef = max(it * 20, k)
        else:
            hnsw_idx.ef = max(it * 200, k)
        print(f"Conducting search with ef={hnsw_idx.ef}...")
        ...
        ######## Gen Command Trace ########
        print("Generating command trace...")
        ...
        print("Begin to conduct simulation...")
        ######## Conduct Simulation ########
        # Use multi-core parallelism for independent sim runs.
        # Note: this stage can be CPU- and memory-intensive; set --workers to control.
        ...
        if workers > 1 and len(cmd_trace) > 1:
            # Force 'fork' on Linux to avoid pickling the whole cmd_trace.
            ctx = mp.get_context("fork")
        ...
        qps_vs_ef_rows.append({
            "dataset_name": dataset_name,
            "recall": recall,
            "qps": qps,
        })
    result_dir = os.path.join(os.path.dirname(__file__), "..", "result", "qps_vs_recall")
    os.makedirs(result_dir, exist_ok=True)

    csv_path = os.path.join(result_dir, f"{dataset_name}_qps_vs_recall.csv")
    with open(csv_path, "w", newline="") as f:
        ...
        writer.writeheader()
        writer.writerows(qps_vs_ef_rows)

    print(f"Saved QPS-vs-RECALL results to {csv_path}")
    print(f"Total wall time: {time.perf_counter() - t_run_start:.3f}s")
